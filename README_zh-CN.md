...
### 🧭 双模式路由系统
- **全站模式 (SPA)**: 基于 Hash 的导航，平滑过渡，侧边栏导航，完整的 Wiki 体验
- **单页模式 (独立页面)**: 通过 `?fullpage=1` 参数访问，干净的分享页面 - 适合外部链接和嵌入
...
| **嵌入能力** | ✅ 通过 `?fullpage=1` 获取干净页面 | ⚠️ 带品牌标识的嵌入 |
...
| 特性 | 描述 |
|------|------|
| **SPA 路由** | 基于 Hash 的导航，无需页面刷新即可无缝浏览 |
| **独立模式** | 在任何 URL 后添加 `?fullpage=1` 获取干净的分享页面 |
| **分类树** | 层级化组织，侧边栏可展开导航 |
| **全文搜索** | 即时搜索所有内容 |
